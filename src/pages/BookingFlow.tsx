...
const BookingFlow = () => {
  ...
  // Service and optionally vendor_id is pre-selected
  const preSelected = location.state as { category?: string; service?: { name: string; price: number; duration: string }; vendor_id?: string } | null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data, error } = await supabase.from("bookings").insert({
        ...
        price: total,
        status: "pending",
        ...(preSelected?.vendor_id ? { vendor_id: preSelected.vendor_id } : {})
      }).select("id").single();
